...
ActionButtonsDelegate::ActionButtonsDelegate(QObject* parent) : QStyledItemDelegate(parent) {}
...
bool ActionButtonsDelegate::editorEvent(QEvent* event, QAbstractItemModel*, const QStyleOptionViewItem& option, const QModelIndex& index) {
    ...
    if (event->type() == QEvent::MouseButtonRelease) {
        ...
        if (deleteRect(option).contains(me->pos())) {
            emit deleteClicked(index.row());
            return true;
        }
        if (editRect(option).contains(me->pos())) {
            emit editClicked(index.row());
            return true;
        }
        if (addRect(option).contains(me->pos())) {
            emit addClicked(index.row());
            return true;
        }
    }
    return false;
