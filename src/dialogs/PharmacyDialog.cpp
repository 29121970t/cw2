...
PharmacyDialog::PharmacyDialog(QWidget* parent)
    ...
    auto mapLabel = new QLabel(tr("Расположение (Google Maps):"), this);
...
void PharmacyDialog::onAccept() {
    if (eName->text().trimmed().isEmpty() || eAddress->text().trimmed().isEmpty() || ePhone->text().trimmed().isEmpty()) {
...
void PharmacyDialog::reverseGeocode(double la, double lo) {
    ...
    QUrlQuery q;
    q.addQueryItem("latlng", QString::number(la, 'f', 6) + "," + QString::number(lo, 'f', 6));
